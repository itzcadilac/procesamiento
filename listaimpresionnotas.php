...
$sql = 'SELECT calend.idecapacitacion, tip.desccapacitacion 
FROM calendcapacitaciones calend 
INNER JOIN tipcapacitaciones tip 
ON calend.idecapacitacion = tip.idecapacitacion 
WHERE tip.estado = 1 
AND calend.hora >= DATE_SUB(NOW(),INTERVAL 30 DAY) 
AND calend.asistenciascerradas = 0 
GROUP BY 1, 2 
...
?>
			  ...
			  <div class="form-group">
					...
					<script>
							$(function(){
								...
								var url = 'busquedahorariosimpresion.php';
								var text = ''
							...
							<button id="btnExportar" class="btn btn-info">
               					 <i class="fas fa-file-excel"></i> Exportar Tabla a Excel
            				</button>
							</div>
							</br>
							<script>
							$(function(){
								$('#horario_r').on('change', function(){
									...
									var url = 'busquedapersonasimpresion.php';
									var text = '';
									$.ajax({
									type:'POST',
									url:url,
									...
									var output = [];
									//['<table id="tabla" class="table table-border table-hover"> <thead>    <tr>      <th width="10%"> DNI - PEX </th>      <th width="20%"> APELLIDOS </th>      <th width="20%"> NOMBRES </th>      <th width="40%"> NOMBRE EMPRESA CONTRATISTA </th>      <th width="5%"> FOTOS </th>      <th width="10%"> QR </th>    </tr>  </thead>  <tbody> ']
									datos.data.forEach(item => {
									//output.push(`<option value="${item.idecalendcapacitaciones}">${item.hora}</option>`);
									output.push(`<tr><td>${item.documento}</td><td>${item.apellidos}</td><td>${item.nombres}</td><td>${item.empresa}</td><td>${item.fotos}</td><td>${item.qr}</td></tr>`);
									});
									...
									//var url = `serv_carga_lista_impresion.php?horario=${horario}`;
									//console.log({url})
								...
								<div>
								<table id="tabla" class="table table-striped table-border table-hover">
  									<thead>
  									  <tr>
  									    <th width="10%"> DNI - PEX </th>
  									    <th width="20%"> APELLIDOS </th>
  									    <th width="20%"> NOMBRES </th>
  									    <th width="30%"> NOMBRE EMPRESA CONTRATISTA </th>
		...
		<script type="text/javascript">
			...
			var combo = document.getElementById("tipcapacitaciones");
			var tipcapa = combo.options[combo.selectedIndex].text;

			var combo2 = document.getElementById("horario_r");
			var fechahora = combo2.options[combo2.selectedIndex].text;
			...
			var pager_selector = "#grid-pager";
				...
				jQuery(grid_selector).jqGrid({
					...
					url:'serv_carga_lista_impresion.php',
					data: grid_data,
					datatype: "json",
					height: 400,
					colNames:['', 'Código', 'Num. Documento', 'Nombres','Apellidos', 'Empresa'],
					colModel:[
						{name:'select',index:'select', width:80, fixed:true, sortable:false, resize:false , hidden: true ,
							...
							formatoptions:{ 
								...
								delOptions:{recreateForm: false, beforeShowForm:beforeDeleteCallback,type: 'POST',datatype: 'json'},
								editOptions:{recreateForm: false, beforeShowForm:beforeEditCallback,type: 'POST',datatype: 'json'}
							}
						},
						{name:'idepersonal', align: "center", index:'idepersonal', width:20, editable: false, hidden: true },
						...
						{name:'apellidos', index:'apellidos', width:60,editable: false,editoptions:{size:"60",maxlength:"150"}},
					...
					editurl: "serv_actualiza_asistencia.php",
					caption: "Lista de Personal que Asistió"
