...
class ValidacionDAO{
   function login($u, $p, $t) {
        ...
        if($t==1)
        {
        $query = "select u.idusuario, u.usuario, u.nomape, u.tipo, p.dscparametro as menu from usuario u INNER JOIN parametro p ON p.txtparametro = u.tipo where u.usuario = '".$u."' AND u.pssword = '".md5($p)."'";
//		$query1 = "update usuario set conectado = 1, ultima_conexion = '".date("Y-m-d H:i:s")."' where usuario like '".$u."'";
        ...
        if($fila=$recordSet->fetch_assoc()) {
            $usuario = new UsuarioVO($fila['idusuario'], $fila['usuario'], "", $fila['nomape'], $fila['tipo'],"","","","","","","",$fila['menu']);
...
//		$query1 = "update usuario set conectado = 1, ultima_conexion = '".date("Y-m-d H:i:s")."' where usuario like '".$u."'";
        ...
        $usuario = "";
        if($fila=$recordSet->fetch_assoc()) {
            $usuario = new UsuarioVO($fila['idecontratista'], $fila['ruc'], "", $fila['razonsocial'], $fila['tipo'], $fila['razonsocial'],$fila['ruc'],"","","","","",$fila['menu']);
            setcookie("id_usuario", $usuario->idUsuario, time()+60*60*24*31,"/","",0);
		//$result = $BD->ejecutar($query1);
		}
        //return $usuario;
        ...
        $query = "select c.id_autorizador, c.nro_personal, c.nombres, c.tipo, p.dscparametro as menu from autorizadores c INNER JOIN parametro p ON p.txtparametro = c.tipo where c.nro_personal = '".$u."' AND c.password = '".md5($p)."'";
//		$query1 = "update usuario set conectado = 1, ultima_conexion = '".date("Y-m-d H:i:s")."' where usuario like '".$u."'";
        $BD = new ConexionDB();
        $recordSet = $BD->ejecutar($query);
        if (!$recordSet){
        ...
        $usuario = "";
        if($fila=$recordSet->fetch_assoc()) {
            $usuario = new UsuarioVO("","","","",$fila['tipo'],"","",$fila['nro_personal'], $fila['id_autorizador'], $fila['nombres'],"","",$fila['menu']);
            setcookie("id_usuario", $usuario->idUsuario, time()+60*60*24*31,"/","",0);
		//$result = $BD->ejecutar($query1);
		}
        //return $usuario;
        ...
        $query = "select c.idecapacitador, c.usuario, c.numdocumento, c.nombres, c.apepaterno, c.apematerno, c.tipo, p.dscparametro as menu from capacitador c INNER JOIN parametro p ON p.txtparametro = c.tipo where c.usuario = '".$u."' AND c.password = '".md5($p)."'";
//		$query1 = "update usuario set conectado = 1, ultima_conexion = '".date("Y-m-d H:i:s")."' where usuario like '".$u."'";
        $BD = new ConexionDB();
        $recordSet = $BD->ejecutar($query);
        if (!$recordSet){
        ...
        $usuario = "";
        if($fila=$recordSet->fetch_assoc()) {
            $usuario = new UsuarioVO($fila['idecapacitador'],$fila['usuario'],"","",$fila['tipo'],"","","","",$fila['nombres'],$fila['apepaterno'],$fila['apematerno'],$fila['menu']);
            setcookie("id_usuario", $usuario->idUsuario, time()+60*60*24*31,"/","",0);
		//$result = $BD->ejecutar($query1);
		}
        //return $usuario;
