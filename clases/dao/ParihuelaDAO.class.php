...
function RegistrarParihuela($requestVO){
    session_start();
    $BD = new ConexionDB();

     
    $pdetail = 1;
    $query = 'INSERT INTO tipo_parihuela (descripcionparihuela, peso) 
    VALUES (UPPER("'.$requestVO->descripcionparihuela.'"), "'.$requestVO->peso.'")';
    $response = $BD->ejecutar($query);
    //echo $query;
    if (!$response){
        $this->error=mysql_error();
