...
$busqmondelez=$_POST['busqmondelez'];

if ($busqmondelez<>''){
	
	$trozos=explode(" ",$busqmondelez);
	$numero=count($trozos);

	if ($numero==1) {
		
		$cadbusca="SELECT pi.documento, pi.ape_paterno,pi.ape_materno,pi.nombres, pi.cargo,pi.empresa, pi.foto,pi.ruc
		from personal_mondelez pi, tipcapacitaciones tc
		where pi.idecapacitacion = tc.idecapacitacion
		and pi.documento = '$busqmondelez' GROUP BY pi.documento;";
